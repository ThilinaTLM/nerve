...
Nerve Sandbox v1 packages the Nerve agent runtime as a durable, isolated container component. An external API, service, or UI can start the container, provide a YAML configuration, and control one or more conversations, agents, and runs over WebSocket without embedding the current Nerve desktop app, local orchestrator, or Web UI.

## Topology

```text
+-------------------------------------------------------------+
| Docker / ECS / Kubernetes / other container runtime          |
|                                                             |
|  +-------------------- Sandbox container ----------------+   |
...
+-----------------------------|---------------------------------+
                              ...
                              v
+-----------------------------+-------------------------------+
| Sandbox manager/controller API                              |
| - authenticates users and external services                  |
| - starts/supervises sandbox containers                       |
| - materializes YAML, secret refs, repo inputs, credentials   |
| - may expose HTTP key-value secret endpoints                 |
| - sends commands and steering                               |
| - receives durable events                                   |
| - persists controller-side state in its own DB/storage       |
+-----------------------------+-------------------------------+
                              | WebSocket / HTTP / app protocol
                              v
+-----------------------------+-------------------------------+
| External UI, CLI, scheduler, or automation                   |
+-------------------------------------------------------------+
```

The sandbox manager/controller is the API-side peer that starts or supervises sandbox containers. It MAY be implemented by the current Nerve orchestrator in the future, but Sandbox v1 does not require that implementation.
...
- optional HTTP key-value secret endpoints authenticated by API key, bearer token, or already-provided OAuth credentials;
...
The sandbox is responsible for validating the YAML, enforcing local policy, resolving credentials lazily, applying Git/GitHub startup setup before boot phases, running the agent/tools, journaling state, refreshing **already-provided** OAuth credential bundles when configured, and redacting secrets. The sandbox MUST NOT perform interactive login flows or ask the model to authenticate to providers. If required credentials are absent, expired without refresh material, invalid, or blocked by network policy, the sandbox reports the affected provider/setup/tool as unavailable or failed.

## Goals

Sandbox v1 aims to provide:

1. **Portable agent runtime**
   - The agent can run in Docker, ECS, Kubernetes, or another container platform.
...
7. **Reproducible customization**
   ...
   - Runtime boot scripts are supported for repository setup and dependency installation, but are less reproducible than build-time image customization.
...
1. **A replacement for all controller behavior**
   - Scheduling containers, billing, multi-tenant quotas, user management, account login, and UI state are controller concerns.
...
| Manager/controller | The external API/service peer that authenticates users/services, starts sandboxes, provides config/credentials, sends commands, receives events, and owns product-specific persistence/UI integration. |
...
| Boot script | Optional shell script or boot phase run after startup setup and before the daemon accepts work. |
...
| Runtime customization | YAML configuration, environment variables, mounted secrets, mounted skills, credential refs, secret stores, and optional boot script. |
...
```text
+--------------------------------------------------------------+
| Controller/API/UI product logic                               |
+--------------------------------------------------------------+
