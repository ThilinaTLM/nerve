...
A sandbox is an isolated workspace plus a persistent `/state` volume. The manager creates lifecycle records and runtime volumes, launches the container/task, accepts the `sandbox_agent` protocol session, forwards targeted catalog operations, ingests the sandbox's durable stream, and serves snapshots to the UI.
...
- Sandbox `/state`: version 4 file-first runtime state, secure launch environment, setup state, run/task state, outbox/ACK, idempotency, checkpoints, and caches.
