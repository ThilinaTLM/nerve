...
- `@nervekit/sandbox-agent`: in-sandbox daemon, tools, tasks, runs, Git, durable state, outbox, and checkpoints.

The manager UI and sandbox daemon use Nerve Protocol v1 shared sessions. Manager lifecycle events use stream `manager`; each sandbox writes `sandbox:<id>`. See [manager](manager.md), [control link](websocket-control.md), [durability](durability.md), [runtime image](runtime-image.md), and [web UI](web-ui.md).
