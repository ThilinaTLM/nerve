...
| Sandbox manager web UI | Dedicated app/package split | `/` is served by `packages/sandbox-manager` from explicit `NERVE_SANDBOX_MANAGER_WEB_DIST`, bundled `dist/web`, or workspace `packages/sandbox-manager-ui/dist`; `/settings` is the dedicated configuration route for all installed pi-ai LLM providers, subscription/API-key auth, provider env/endpoint overrides, Git/GitHub, Jira, Confluence, and web search. The UI lives in `packages/sandbox-manager-ui` and shares primitives/theme through `packages/shared-ui`; `packages/web` is local workbench only. Ships fleet dashboard + filters/search, lifecycle controls, create dialog, settings/credential control, detail tabs, and manager-mediated chat. REST/WS clients validate with shared schemas and connect only to the manager. The default manager UI API-key cookie remains loopback-only; AWS/proxy deployments can opt into `trusted_proxy` mode for HTTPS requests from configured proxy CIDRs. |
...
pnpm --filter @nervekit/shared-ui check
