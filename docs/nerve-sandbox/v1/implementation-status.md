...
| Image build/smoke | Guarded | Dockerfile bundles a generic development toolchain with build-time availability checks, and smoke coverage detects Docker/Podman at runtime and skips clearly when unavailable. Full operator smoke requires a configured runtime image. |
