...
| Sandbox manager web UI | Dedicated app/package split | `/sandbox-manager` is served by `packages/sandbox-manager` from explicit `NERVE_SANDBOX_MANAGER_WEB_DIST`, bundled `dist/web`, or workspace `packages/sandbox-manager-ui/dist`. The UI now lives in `packages/sandbox-manager-ui` and shares primitives/theme through `packages/ui`; `packages/web` is local workbench only. Ships fleet dashboard + filters/search, lifecycle controls, create dialog, credential control center, detail tabs, and manager-mediated chat. REST/WS clients validate with shared schemas and connect only to the manager. Loopback `HttpOnly` cookie auth issued by the manager static handler; remote auth stays external/proxy-backed. |
