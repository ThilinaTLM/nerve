...
- Conversation rendering uses `presentation/state/conversation-event-reducer.ts` for dispatch and sequence/revision checks, with focused snapshot, live-content, and compaction modules. Live handlers retain the existing copy-on-write draft; ephemeral notifications never advance durable watermarks.
...
- ToolService receives named host capabilities and persistence dependencies from bootstrap. Approval/question projections and startup/result-payload reconciliation have focused owners; lifecycle-local execution wiring, durable transitions, event publication, and waiters remain coordinated by ToolService.
