...
| Clipboard image paste                  | Give image-capable models local image input                                 | `handlePaste`; `saveClipboardImage`                          | Paste-only; uploaded images become OS-temp file paths. No file picker, generic attachment UI, thumbnail, or drag/drop. MIME whitelist exists, but no documented size/signature guarantee. | P1       |
...
| Read-only, supervised, autonomous permissions | Match agent authority to risk tolerance                    | policy evaluator and tests                        | Read-only blocks mutation, command/Python, ordinary network, and Explore. Supervised approves non-read risks; selected read tools may be autoapproved.                                          | P0       |
...
| Jira                        | Read and update delivery work                       | Jira tool catalog                                  | Module/credential gated; lookup/search plus issue/project create/update/comment/transition; mutations are supervised by tool policy.                                                       | P1       |
| Confluence                  | Read and publish project knowledge                  | Confluence tool catalog                            | Module/credential gated; spaces/pages/search/download/create/update/publish/attachments; narrow result limits and artifacts.                                                               | P1       |
...
| Local storage and cleanup      | Understand backup and disk use                             | storage domains/state marker              | `NERVE_HOME` default `~/.nerve`; file-first state with rebuildable indexes/cache; cleanup skips symlinks and protects active work.                                                           | P0       |
...
| `/guides/composer/`              | Send/stop, keyboard behavior, `/` commands, `@` paths, queue/cancel-edit, context/todos                                 | First project → images/voice, controls                | Toolbar + completion + queued prompt sequence; no generic file upload/drag-drop claim               |
| `/guides/images-and-voice/`      | Paste image paths, supported clipboard MIME behavior, model compatibility, recording/transcription                      | Composer + compatible model/OAuth → model guide       | Paste/voice states; temp-file, external transcription, 8m/25MB warning                              |
| `/guides/agent-controls/`        | Coding/planning, read-only/supervised/autonomous, read autoapproval, live model/thinking changes                        | Provider setup → approvals, security                  | Composer controls screenshot; “next model request” wording                                          |
...
| `/developers/persistence-security/` | file-first state, event logs/indexes, secrets, auth, Electron boundary, migration                                                                                                         | Architecture → protocol/security           | Storage/trust diagram; schema allowance is not a redaction guarantee                        |
...
| Composer attachments                                      | State clipboard image paste only. Do not claim file picker, drag/drop, thumbnail attachment, or general file upload.                                                                                      |
