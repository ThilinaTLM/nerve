...
| Backpressure | Partial | Per-session queueing, adaptive batching, queue metrics, transient overflow dropping, and flow transitions are implemented; coalescing policy is conservative. |
| HTTP mapping | Implemented for first wave | `POST /api/protocol/v1` supports snapshot, approval, user question, and plan review methods with protocol response/error envelopes. |
| Idempotency | Implemented for first wave | Recommended mutation idempotency keys are cached in-memory with conflict detection. |
...
- [x] Transient overflow may be dropped with counters under pressure.
- [x] Selected HTTP methods use protocol response/error envelopes.
