...
This overview defines the conceptual model. Normative wire shapes are in [Message Envelope](./message-envelope.md), [Session Lifecycle](./session-lifecycle.md), [Event Stream](./event-stream.md), [Replay and Acknowledgements](./replay-and-ack.md), [Backpressure](./backpressure.md), and [Errors and Security](./errors-and-security.md). [Feature Coverage](./feature-coverage.md) maps current Nerve features to v1, [HTTP Mapping](./http-mapping.md) describes optional HTTP envelope use, [Extension Model](./extension-model.md) defines future evolution rules, and [Implementation Guide](./implementation-guide.md) describes rollout guidance. Concrete examples are in [Examples](./examples.md).
