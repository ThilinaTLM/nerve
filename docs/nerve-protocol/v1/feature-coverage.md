...
| Tool calls                 | `GET /api/tool-calls`, `GET /api/tool-calls/:id`                                           | REST/resource; RPC candidate                            | State changes are visible through `conversation.tool_call.updated`, `approval.*`, and related events.                           |
...
| User questions             | `/api/user-questions/*`                                                                    | REST/resource; strong RPC candidate                     | Answer/dismiss are small mutation methods and publish `user_question.*`.                                                        |
| Plan reviews               | `/api/plan-reviews/*`                                                                      | REST/resource; strong RPC candidate                     | Review decisions are small mutation methods and publish `plan_review.*`.                                                        |
...
| `conversation.tool_call.updated`                                                                                                                  | durable                                                         | Tool call transcript and approval state                   | Upsert by tool call ID.                                                                                              |
...
| `task.*`                                                                                                                                          | durable for lifecycle, transient only for non-required progress | Task list, foreground task, notifications                 | `task.log` is durable enough for log query/replay when needed; high-volume output may use paginated log storage.     |
...
| `user_question.*`                                                                                                                                 | durable                                                         | Human input UX and notifications                          | Pending questions must be recoverable.                                                                               |
| `plan_review.*`                                                                                                                                   | durable                                                         | Plan review UX and notifications                          | Pending plan reviews must be recoverable.                                                                            |
