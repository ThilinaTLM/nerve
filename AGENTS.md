...
- Use the official shadcn-svelte components in `packages/web/src/lib/components/ui`; style only with the shadcn tokens (+ `success`/`warning`/`info`), never hard-coded colors; use `@lucide/svelte` for icons and keep mono for code/logs/paths only. The theme lives in `packages/web/src/app.css`.
