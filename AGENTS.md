- Nerve defaults portable state to `~/.nerve`, daemon HTTP to `3747`, and mobile HTTPS to `3748` when enabled. Never debug or run test servers against the live home; use a copied or fresh `NERVE_HOME` under `/tmp` and explicit ports. Desktop tests requiring full isolation must also use a separate Electron `userData` profile outside `NERVE_HOME`.
