<script lang="ts">
import type { ConfluenceIncludedCountsPayload } from "@nervekit/contracts";
...
{#if metrics.length > 0}
  <div class="flex flex-wrap gap-1.5">
    {#each metrics as metric (metric.label)}
      <span
        class="inline-flex items-center gap-1.5 rounded-sm border bg-sidebar px-2 py-1 text-xs"
      >
        <span class="font-semibold tabular-nums text-sidebar-foreground"
