...
const stats = $derived.by(() => {
  ...
  if (cumulative.cacheRead)
    parts.push(`R${formatTokens(cumulative.cacheRead)}`);
  if (cumulative.cacheWrite)
    parts.push(`W${formatTokens(cumulative.cacheWrite)}`);
...
{#if window > 0 || stats.length > 0}
  <span class="footer-chip context-chip" {title}>
    ...
    {/each}
    {#if window > 0}
      <span class="ctx-context" data-tone={tone}
        >{percentLabel}/{windowLabel}</span
      >
    {/if}
  </span>
{/if}
