<script lang="ts">
import type { TaskLogQueryResponse } from "@nervekit/contracts";
...
>
  {#if command}
    <pre
      class="mb-2 whitespace-pre-wrap break-words text-foreground">$ {command}</pre>
  {/if}
  ...
  {:else}
    <div class="grid gap-0">
      {#each taskLogs?.events ?? [] as event (event.seq)}
        <pre
          class={`whitespace-pre-wrap break-words ${lineClass(event)}`}><TerminalText
            text={event.line}
            stream={event.stream}
            level={event.level}
          /></pre>
