...
test("targeted lookups skip historical hydration and evict terminal commits", async (t) => {
  const stateDir = await mkdtemp(
    path.join(os.tmpdir(), "nerve-sandbox-run-lookup-"),
  );
  t.after(() => rm(stateDir, { recursive: true, force: true }));
  let id = 1000;
  const ids = { next: () => String(++id) };
  ...
  const seed = new SandboxRunUnitOfWork(stateDir, 0);
  ...
  const restarted = new SandboxRunUnitOfWork(stateDir, 0);
  ...
  await assert.rejects(restarted.list(), /Invalid JSONL record/);
