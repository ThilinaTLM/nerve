...
import { logLevelTone } from "@nervekit/workbench-ui/core/utils/status";
...
const sources: Array<ApplicationLogSource | "all"> = [
  "all",
  "orchestrator",
  "desktop",
  "web",
...
function hasDetail(log: LogRow): boolean {
  return (
    Boolean(log.error) || contextEntries(log).length > 0 || refs(log).length > 0
  );
}
      ...
      <div class="segmented" role="group" aria-label="Log source filter">
        {#each sources as option (option)}
          <button
            ...
            onclick={() => (source = option)}>{option}</button
          >
        {/each}
      </div>
      <div class="toolbar-actions">
        <span class="result-count"
          >{rows.length} {rows.length === 1 ? "entry" : "entries"}</span
        >
        {#if filtersActive}
        ...
        >
          <RefreshCw size={13} class={loading ? "animate-spin" : ""} />{loading
            ? "Loading"
            : "Refresh"}
        </Button>
        <Button
          ...
          variant="ghost"
          onclick={copyLogs}
          disabled={rows.length === 0 || pruning}
        >
          <Copy size={13} />Copy
        </Button>
        <Button
      ...
      {#each rows as log (log.id)}
        ...
        <article class={`log-row ${log.level}`} class:open>
          <div class="log-line" data-detail={detail ? "" : undefined}>
            ...
            <span class="source" title={`${log.source}/${log.component}`}
              >{log.source}/{log.component}</span
            >
          ...
          {#if detail && open}
            <div class="log-detail">
              ...
              {/if}
              {#if log.error}
                <pre class="error-block">{log.error.stack ??
                    log.error.message}</pre>
              {/if}
            </div>
          {/if}
        </article>
