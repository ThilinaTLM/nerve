...
import { Card } from "@nervekit/workbench-ui/components/ui/card";
...
let discardCandidate = $state<
  { repo: string; file: GitFileChange } | undefined
>(undefined);
...
const current = $derived(
  projectState?.repoStates[gitRepoStateKey(selectedRepo)],
);
...
const selectedRepoHasGithubRemote = $derived(
  Boolean(
    selectedRepoSummary?.hasRemote && selectedRepoSummary.hasGithubRemote,
  ),
);
...
const stagedFiles = $derived(
  changes?.files.filter((file) => file.staged) ?? [],
);
...
async function onCreateBranch(repo: string) {
  ...
  const created = await createGitRepoBranch(
    activeProject.id,
    repo,
    newBranchName,
  );
...
<div class="p-2">
  ...
  {:else if repos.length === 0}
    <p class="px-1 py-6 text-center text-xs text-muted-foreground">
      No git repositories found in this directory (searched up to 2 levels
      deep).
    </p>
  {:else}
      ...
      {#if discoverError}
        <Card class="gap-0 overflow-hidden p-0">
          <div class="px-3 py-2 text-xs text-warning">
            Using cached Git data. Refresh failed: {discoverError}
          </div>
        </Card>
      {/if}
