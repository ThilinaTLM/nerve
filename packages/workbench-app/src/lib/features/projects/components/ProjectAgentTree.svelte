...
>
  ...
  {#each groups as group (group.key)}
    <ContextMenu
      items={buildProjectMenu(group.project, menuContext)}
      triggerClass="project-context-trigger"
    >
      <PanelSection
        title={group.label}
        icon={Folder}
  ...
  <ProjectConversationsDialog
    ...
    buildMenu={(conversation) =>
      pendingConversations
        ? buildConversationMenu(
            pendingConversations.project,
            conversation,
            menuContext,
          )
        : []}
    onOpenChange={(open) => {
      if (!open) pendingConversations = undefined;
    }}
  />
{/if}
