...
const confluenceSettings = $derived(
...
const tokenConfigured = $derived(
  confluenceProvider?.configured &&
    confluenceProvider.credentialType === "api_key",
);
const hasRequiredConfig = $derived(
  Boolean(
    confluenceSettings.siteUrl && confluenceSettings.email && tokenConfigured,
  ),
);
...
async function removeToken() {
  ...
  try {
    ...
    onSettingsChange?.(
      { tools: { confluence: { enabled: false } } },
      { immediate: true },
    );
...
>
  ...
  <ul class="settings-tool-list" aria-label="Confluence tools">
    {#each confluenceTools as tool (tool.name)}
    ...
    <div class="settings-copy">
      ...
      <span>
        {#if confluenceSettings.siteUrl && confluenceSettings.email}
          {confluenceSettings.siteUrl} · {confluenceSettings.email}{confluenceSettings.defaultSpaceKey
            ? ` · ${confluenceSettings.defaultSpaceKey}`
            : ""}
        {:else}
  ...
  <div class="settings-credential-summary">
    <div class="settings-copy">
      ...
      <span
        >{tokenConfigured
          ? "•••••••• configured"
          : "Required before Confluence tools can be enabled."}</span
      >
    </div>
    {#if tokenConfigured}
      <Button
...
<Dialog.Root bind:open={dialogOpen}>
  <Dialog.Content class="settings-runtime-dialog">
    <Dialog.Header>
      ...
      <Dialog.Description>
        Store Confluence Cloud connection details. The API token is encrypted
    ...
    >
      <label class="settings-key-label" for="tools-confluence-site-url">
        ...
        <Input
          id="tools-confluence-site-url"
          bind:value={siteUrlDraft}
      ...
      <label class="settings-key-label" for="tools-confluence-token">
        ...
        <Input
          ...
          placeholder={tokenConfigured
            ...
            : "Paste your Confluence API token"}
          disabled={busy}
        />
      </label>
      <label class="settings-key-label" for="tools-confluence-space-key">
        ...
        <Input
          id="tools-confluence-space-key"
          bind:value={spaceKeyDraft}
          placeholder="Optional, e.g. DEV"
          disabled={busy}
        />
      </label>
      ...
      {#if tokenConfigured}
        <p class="settings-dialog-note">
          Current token: <code>•••••••• configured</code>
        </p>
      {/if}
