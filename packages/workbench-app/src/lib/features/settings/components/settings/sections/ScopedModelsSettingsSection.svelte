...
<SettingsSectionCard section="models" title="Scoped models">
  ...
  {:else if !scopeActive}
    <div class="scoped-empty">
      ...
      <span
        >Every authenticated model appears in the composer picker. Add models to
        narrow it down.</span
      >
    </div>
  {:else}
          ...
          <span class="scoped-list-text">
            <strong
              >{entry.model
                ...
                : entry.selection.modelId}</strong
            >
            <span
              >{providerDisplayName(entry.selection.provider)} · {entry
                .selection.modelId}</span
            >
          </span>
          {#if entry.stale}
    ...
    {#if staleCount > 0}
      <p class="settings-note">
        {staleCount} scoped {staleCount === 1 ? "model is" : "models are"} no longer
        available and will be ignored by the picker.
      </p>
    {/if}
  {/if}
</SettingsSectionCard>
