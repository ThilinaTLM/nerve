...
function applyOperation(next: StorageCleanupOperation | null) {
  ...
  if (next.status === "succeeded") {
    notify.success(
      next.results.some((result) => result.outcome === "failed")
        ? "Cleanup completed with issues"
        : `Freed ${formatBytes(next.freedBytes)}`,
    );
...
>
  {#snippet actions()}
    <Button
      ...
      disabled={loading ||
        refreshing ||
        (operation?.cancellable === false && active)}
    ...
    <Button
      size="sm"
      disabled={!usage || active || operationLoading}
      onclick={openCleanup}
    >
  ...
  {:else if usage}
    <div class="grid gap-4">
      <section
        class="grid gap-4 rounded-lg border bg-card p-4 md:grid-cols-[minmax(0,1fr)_auto]"
      >
        <div class="min-w-0">
        ...
        <div
          class="self-end text-left text-xs text-muted-foreground md:text-right"
        >
          Calculated {new Date(usage.generatedAt).toLocaleString()}
        </div>
        <div
          class="col-span-full flex h-3 overflow-hidden rounded-full bg-muted"
          ...
          {#if !active && operation.results.length > 0}
            <ul class="divide-y rounded-md border">
              {#each operation.results as result (result.target)}
                <li
                  class="grid gap-1 px-3 py-2.5 sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center"
                >
                  <div class="min-w-0">
                    <div class="flex items-center gap-2">
                      <span class="text-sm font-medium"
                        >{targetLabel(result.target)}</span
                      >
                      <Badge
                        size="sm"
                  ...
                  <div
                    class="font-mono text-xs text-muted-foreground sm:text-right"
                  >
                    {formatBytes(result.freedBytes)}
                    {#if result.removedItems > 0}
                      · {result.removedItems} removed{/if}
                    {#if result.skipped > 0}
                      · {result.skipped} skipped{/if}
                  </div>
                </li>
              {/each}
            </ul>
        ...
        <section class="overflow-hidden rounded-lg border bg-card">
          <div class="border-b px-4 py-3">
            ...
            <p class="mt-0.5 text-xs text-muted-foreground">
              Categories are ordered by current footprint.
            </p>
          </div>
          <ul class="divide-y">
            {#each categories as category, index (category.key)}
              <li
                class="grid grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-3 px-4 py-3"
              >
                <span
                  class="size-2 rounded-full"
                  style={`background: var(${storageTones[index % storageTones.length]});`}
                ></span>
                <div class="min-w-0">
                  <div class="flex flex-wrap items-center gap-2">
                    <span class="text-sm font-medium">{category.label}</span>
                    {#if category.protected}<Badge size="sm" variant="secondary"
                        >Protected</Badge
                      >{/if}
                    {#if category.cleanable}<Badge size="sm" variant="outline"
                        >Cleanable</Badge
                      >{/if}
                  </div>
                  <p
                  ...
                  >
                    {category.fileCount.toLocaleString()} files · {percent(
                      category.bytes,
                    )}%
                  </p>
                </div>
                <span class="font-mono text-xs font-medium"
                  >{formatBytes(category.bytes)}</span
                >
              </li>
            {/each}
          </ul>
        ...
        <div class="grid content-start gap-3">
          <section class="rounded-lg border bg-card p-4">
            ...
            <p class="mt-2 text-2xl font-semibold">
              {formatBytes(
                usage.sqlite.dbBytes +
                  usage.sqlite.walBytes +
                  usage.sqlite.shmBytes,
              )}
            </p>
            <p class="mt-1 text-xs text-muted-foreground">
              Current records and retained event logs can be rebuilt into a
              fresh compact index.
            </p>
          </section>
          ...
          <section class="rounded-lg border bg-card p-4">
            ...
            <p class="mt-1 text-2xl font-semibold">
              {usage.conversations.total.toLocaleString()}
            </p>
                ...
                {#each usage.conversations.largest.slice(0, 3) as conversation (conversation.conversationId)}
                  <li class="flex min-w-0 justify-between gap-3 text-xs">
                    <span
                      ...
                      >{conversation.title ?? conversation.conversationId}</span
                    >
  ...
  <div class="grid gap-5 p-4">
    ...
    >
      <div>
        ...
        <p class="text-xs text-muted-foreground">
          {footprint.upTo ? "Up to " : ""}{formatBytes(footprint.bytes)} current footprint
        </p>
      </div>
    ...
    <section class="grid gap-2">
      ...
      <div
        class="grid gap-3 rounded-md border p-3 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center"
      >
        <Checkbox
          id="cleanup-conversations"
          bind:checked={selection.conversations}
          aria-label="Remove old conversations"
        />
        <label for="cleanup-conversations" class="cursor-pointer"
          ><span class="block text-sm font-medium">Old conversations</span><span
            class="block text-xs text-muted-foreground"
            >Not updated recently · {targetFootprint("conversations")}</span
          ></label
        >
        <div class="flex items-center gap-2 pl-7 sm:pl-0">
          <Input
            ...
            min={1}
            max={3650}
            value={selection.conversationsDays}
            disabled={!selection.conversations}
            ariaLabel="Conversation age in days"
            class="w-20"
            oninput={(event) =>
              (selection.conversationsDays = Number(
    ...
    <section class="grid gap-2">
      ...
      <div class="divide-y rounded-md border">
        <div
          class="grid gap-3 p-3 sm:grid-cols-[auto_minmax(0,1fr)_auto] sm:items-center"
        >
          <Checkbox
            id="cleanup-dated-logs"
            bind:checked={selection.datedLogs}
            aria-label="Remove old dated logs"
          />
          <label for="cleanup-dated-logs" class="cursor-pointer"
            ><span class="block text-sm font-medium">Dated log files</span><span
              class="block text-xs text-muted-foreground"
              >Application and desktop logs · {targetFootprint(
                "datedLogs",
              )}</span
            ></label
          >
          <div class="flex items-center gap-2 pl-7 sm:pl-0">
            <Input
              ...
              min={1}
              max={3650}
              value={selection.logsDays}
              disabled={!selection.datedLogs}
              ariaLabel="Log age in days"
              class="w-20"
              oninput={(event) =>
                (selection.logsDays = Number(
                  (event.currentTarget as HTMLInputElement).value,
                ))}
            /><span class="text-xs text-muted-foreground">days</span>
          </div>
    ...
    <section class="grid gap-2">
      <div>
        <h3 class="text-sm font-medium">Disposable data</h3>
        <p class="text-xs text-muted-foreground">
    ...
    <section class="grid gap-2">
      <div>
        <h3 class="text-sm font-medium">Search index</h3>
        <p class="text-xs text-muted-foreground">
          Rebuild the query cache instead of vacuuming the large database in
          place.
        </p>
      </div>
      ...
      {#if selection.searchIndex}<p
          class="rounded-md border border-warning/40 bg-warning/10 p-3 text-xs text-warning"
        >
          Older searchable event history that exists only in the index will be
          dropped. Conversation files are not removed by this option.
        </p>{/if}
    </section>

    {#if selection.conversations}<p
        class="rounded-md border border-destructive/40 bg-destructive/10 p-3 text-xs text-destructive"
      >
        Old conversations and their messages are permanently deleted and cannot
        be recovered.
      </p>{/if}
    {#if selectionError}<p class="text-sm text-destructive">
        {selectionError}
      </p>{/if}
  </div>

  {#snippet footer()}
    <Button size="sm" variant="outline" onclick={() => (cleanupOpen = false)}
      >Cancel</Button
    >
    <Button
      size="sm"
