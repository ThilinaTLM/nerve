...
>
  ...
  <ul class="settings-tool-list" aria-label="Jira tools">
    {#each jiraTools as tool (tool.name)}
    ...
    <div class="settings-copy">
      ...
      <span>
        {#if jiraSettings.siteUrl && jiraSettings.email}
          {jiraSettings.siteUrl} · {jiraSettings.email}{jiraSettings.defaultProjectKey
  ...
  <div class="settings-credential-summary">
    <div class="settings-copy">
      ...
      <span
        >{tokenConfigured
          ? "•••••••• configured"
          : "Required before Jira tools can be enabled."}</span
      >
    </div>
    {#if tokenConfigured}
      <Button
...
<Dialog.Root bind:open={dialogOpen}>
  <Dialog.Content class="settings-runtime-dialog">
    <Dialog.Header>
      ...
      <Dialog.Description>
        Store Jira Cloud connection details. The API token is encrypted before
    ...
    >
      ...
      <label class="settings-key-label" for="tools-jira-project-key">
        ...
        <Input
          id="tools-jira-project-key"
          bind:value={projectKeyDraft}
