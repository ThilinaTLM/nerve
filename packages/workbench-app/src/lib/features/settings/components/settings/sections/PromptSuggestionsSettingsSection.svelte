...
>
  <p class="settings-note">
    JavaScript trust is tied to the predicate content hash. Editing the
    predicate requires approval again.
  </p>

  {#if statuses.length === 0}
    <p class="text-sm text-muted-foreground">
      No prompt suggestions found for the current project or user suggestions
      directory.
    </p>
  {:else}
                ...
                <Badge
                  variant={status.status === "allowed"
                    ...
                    : status.status === "denied"
                      ? "destructive"
                      : "secondary"}>{labelForStatus(status.status)}</Badge
                >
                <span class="text-xs text-muted-foreground"
                  >{status.sourceKind}</span
                >
              </div>
            ...
            {#if status.requiresTrust && status.trustId}
              <div class="flex flex-wrap gap-1">
                <Button
                  ...
                  onclick={() => update(status.trustId, "allowed")}
                  >Allow</Button
                >
                <Button
                  size="xs"
                  variant="destructive"
                  disabled={savingTrustId === status.trustId}
                  onclick={() => update(status.trustId, "denied")}>Deny</Button
                >
                <Button
                  size="xs"
                  variant="ghost"
                  disabled={savingTrustId === status.trustId}
                  onclick={() => update(status.trustId, "unset")}>Reset</Button
                >
              </div>
            {/if}
          </div>
