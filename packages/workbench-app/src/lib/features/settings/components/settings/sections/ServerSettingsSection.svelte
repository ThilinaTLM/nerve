...
<SettingsSectionCard section="server" title="Server">
  <div class="settings-field-grid">
    <label>
      ...
      <Input
        ...
        oninput={(event) =>
          updateHost((event.currentTarget as HTMLInputElement).value)}
      />
    </label>
    <label>
      <span>Port</span>
      <Input
        ...
        oninput={(event) =>
          updateServerPort((event.currentTarget as HTMLInputElement).value)}
      />
    </label>
  </div>
  ...
  <p class="settings-note">
    Restart the daemon after changing host, port, or remote access.
  </p>
</SettingsSectionCard>
