...
const fileEditingTools: ToolSummary[] = [
  {
    name: "write",
    description: "Create or overwrite files when workspace writes are allowed.",
  },
  {
    name: "edit",
    description:
...
function setToolsEnabled(names: ConfigurableToolName[], enabled: boolean) {
  ...
  const next = new SvelteSet(settingsDraft.tools.disabled);
...
{#snippet webSwitch()}
  <ToggleSwitch
    ...
    onCheckedChange={(checked) =>
      setToolsEnabled(["web_search", "web_fetch"], checked)}
  />
{/snippet}
...
>
  {#snippet actions()}{@render alwaysOnSwitch(
      "File inspection tools are always enabled",
    )}{/snippet}
...
>
  {#snippet actions()}{@render alwaysOnSwitch(
      "File editing tools are always enabled",
    )}{/snippet}
...
>
  {#snippet actions()}{@render alwaysOnSwitch(
      "Plan mode tools are always enabled",
    )}{/snippet}
  ...
  <div class="settings-credential-summary">
    <div class="settings-copy">
      ...
      <span
        >{tavilyConfigured
          ? "•••••••• configured"
          : "Required for web_search."}</span
      >
    </div>
    <Button size="sm" variant="outline" onclick={openTavilyDialog}
...
>
  {#snippet actions()}{@render alwaysOnSwitch(
      "Task management tools are always enabled",
    )}{/snippet}
    ...
    <div class="settings-copy">
      ...
      <span>
        {#if python?.available}
          ...
          <code>{python.executable ?? "No executable"}</code>
        {:else}
          {python?.error ?? "No Python runtime was detected."}
        {/if}
      </span>
    </div>
    <Button size="sm" variant="outline" onclick={openPythonRuntimeDialog}
      >Configure runtime</Button
    >
  </div>

  <p class="settings-note">
    Planning-mode Python runs with file-write guardrails. This is not a hard
    security sandbox.
  </p>
</SettingsSectionCard>
...
<Dialog.Root bind:open={tavilyDialogOpen}>
  <Dialog.Content class="settings-runtime-dialog">
    <Dialog.Header>
      ...
      <Dialog.Description>
        Store a Tavily API key for the web_search tool. The key is encrypted
    ...
    >
      ...
      {#if tavilyConfigured}
        <p class="settings-dialog-note">
          Current key: <code>•••••••• configured</code>
        </p>
      {/if}
      ...
      <Dialog.Footer>
        <Button
          type="button"
          variant="ghost"
          onclick={() => (tavilyDialogOpen = false)}>Close</Button
        >
        {#if tavilyConfigured}
          <Button
            type="button"
            variant="outline"
            disabled={tavilyBusy}
            onclick={() => (removeTavilyOpen = true)}>Remove key</Button
          >
        {/if}
        <Button
          type="submit"
...
<Dialog.Root bind:open={pythonDialogOpen}>
  <Dialog.Content class="settings-runtime-dialog">
    <Dialog.Header>
      ...
      <Dialog.Description>
        Set a manual Python executable path, or leave it empty to auto-detect
        from the project and system PATH.
      </Dialog.Description>
    </Dialog.Header>
    ...
    <div class="settings-dialog-body">
      ...
      {#if python?.executable}
        <p class="settings-dialog-note">
          Current executable: <code>{python.executable}</code>
        </p>
      {/if}
    </div>

    <Dialog.Footer>
      <Button variant="ghost" onclick={() => (pythonDialogOpen = false)}
        >Cancel</Button
      >
      <Button variant="outline" onclick={resetPythonPath}
