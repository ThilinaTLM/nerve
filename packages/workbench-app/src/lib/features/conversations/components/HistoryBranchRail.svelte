...
const rootColor = $derived(
  rootOnActivePath ? "var(--primary)" : "var(--muted-foreground)",
);
...
function entryMenu(node: ConversationTreeNode): ContextMenuItem[] {
  const items: ContextMenuItem[] = [
    {
      label: "Jump here",
      icon: ArrowRight,
      onSelect: () => onNavigateToEntry?.(node.entry.id),
    },
    {
  ...
  if (node.entry.role === "user") {
    items.push({
      label: "Edit & resend",
      icon: Pencil,
      onSelect: () => onEditEntry?.(node.entry),
    });
  }
  items.push(
    { type: "separator" },
  ...
  <div class="relative" style={`min-height:${graphHeight}px`}>
    ...
    >
      ...
      {#each nodes as node (node.visibleIndex)}
        {@const cx = laneX(node.lane)}
        {@const cy = entryRowY(node.visibleIndex)}
        {@const color = node.isOnActivePath
          ? "var(--primary)"
        ...
        {#each items as item (item.type === "entry" ? item.row.node.entry.id : `seg:${item.segment.id}`)}
          {#if item.type === "segment"}
            ...
            <button
              ...
              onclick={() =>
                onSelect({ kind: "segment", segment: item.segment })}
            >
              <span
              ...
              >
                {#if isOpen}<ChevronDown
                    class="size-4"
                    strokeWidth={2}
                  />{:else}<ChevronRight class="size-4" strokeWidth={2} />{/if}
              </span>
            </button>
          {:else}
            {@const row = item.row}
            ...
            >
              ...
              >
                ...
                <span
                  class="font-mono text-xs tabular-nums text-muted-foreground/50"
                >
                ...
                {#each desc.badges as badge, b (b)}
                  ...
                  <span
                    class={`flex shrink-0 items-center gap-0.5 ${HISTORY_TONE_TEXT[badge.tone]}`}
                ...
                {/each}
                <span
                  class="shrink-0 font-mono text-xs text-muted-foreground/50"
                >
