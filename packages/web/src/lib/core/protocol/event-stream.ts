...
export function globalProcessedSeqFromCursor(cursor: {
  streams: Array<{ stream: string; processedSeq: number }>;
}): number {
  return (
    cursor.streams.find((stream) => stream.stream === "global")?.processedSeq ??
    0
  );
}
...
export function resetClientEventStreamStateFromCursor(
  ...
  const processedSeq = globalProcessedSeqFromCursor(cursor);
