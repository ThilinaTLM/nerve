...
<div class="flex min-w-0 items-center gap-2.5 rounded-sm border bg-sidebar px-2.5 py-2">
  <span
  ...
  <div class="grid min-w-0 flex-1 gap-0.5">
    <div class="flex min-w-0 flex-wrap items-center gap-x-2 gap-y-1">
      <span class="min-w-0 truncate text-xs font-medium text-sidebar-foreground">{name}</span>
      {#if user.emailAddress && user.emailAddress !== name}
        <span class="min-w-0 truncate font-mono text-xs text-muted-foreground">{user.emailAddress}</span>
      {/if}
      {#if user.accountType}
    ...
    <span class="truncate font-mono text-xs text-muted-foreground/80">{user.accountId}</span>
  </div>
</div>
