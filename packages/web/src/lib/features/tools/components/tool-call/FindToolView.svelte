...
{:else}
  <ul class="m-0 list-none rounded-sm border bg-sidebar px-2.5 py-2 font-mono text-xs leading-snug text-sidebar-foreground">
    {#each visible as item (item.path)}
      <li><button type="button" class="cursor-pointer border-0 bg-transparent p-0 text-left font-mono text-xs text-primary hover:underline" onclick={() => onOpenFile?.(item.openPath)}>{item.path}</button></li>
    {/each}
  </ul>
{/if}
