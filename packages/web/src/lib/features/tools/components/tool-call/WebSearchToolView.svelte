...
{#if results.length > 0}
  <ul class="m-0 grid list-none gap-0 divide-y overflow-hidden rounded-sm border bg-sidebar p-0">
      ...
      <li>
        ...
        >
          <span class="flex min-w-0 items-baseline gap-2">
            <span class="w-4 shrink-0 text-right font-mono text-xs tabular-nums text-muted-foreground/70">{index + 1}</span>
            <span class="min-w-0 flex-1 truncate text-xs font-medium text-foreground group-hover:underline">{result.title}</span>
            <span class="shrink-0 font-mono text-xs text-muted-foreground">{hostname(result.url)}</span>
          </span>
          {#if result.content}
            <span class="line-clamp-2 pl-6 text-xs leading-snug text-muted-foreground">{result.content}</span>
          {/if}
        </a>
      </li>
    {/each}
