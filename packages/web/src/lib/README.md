...
- `core/` owns workbench-local primitives: event bus/websocket transport, aggregated state types (`core/types/state-types.ts`), state keys, hooks, shortcuts, audio, highlighting, clipboard bridge, and workbench utilities. Shared UI primitives, generic API helpers, Markdown, and PlainText live in `@nervekit/ui`.
