...
- Slice modules (`workspace.svelte.ts`, `conversation-flow.svelte.ts`, `settings.svelte.ts`, `composer-config.svelte.ts`, and `workbench/processes.svelte.ts`) own domain actions.
