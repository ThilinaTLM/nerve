...
function generateSandboxConfigJsonSchema(): string {
  ...
  const schema = {
    ...
    description:
      "JSON Schema for Nerve sandbox-agent YAML/JSON config. Generated from packages/shared/src/domains/sandbox/sandbox.config.schema.ts; runtime Zod validation remains authoritative for cross-field hardening rules.",
    ...generated,
  };
...
async function main(): Promise<void> {
  ...
  if (process.argv.includes("--check")) {
    ...
    if (actual !== expected) {
      throw new Error(
        `Generated sandbox config schema is stale. Run pnpm --filter @nervekit/shared schema:generate.`,
      );
    }
    return;
  }
