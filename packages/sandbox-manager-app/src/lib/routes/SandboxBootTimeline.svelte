...
const setupPhases = $derived<
  Array<{ phase: string; status: StartupSetupStatus }>
>(
  setup
    ? [
...
<ScrollArea class="h-full">
  <div class="flex flex-col gap-4 p-4">
    <Card class="rounded-md border">
      <CardHeader
        ><CardTitle class="text-sm">Setup status</CardTitle></CardHeader
      >
            ...
            <div class="flex items-start gap-2">
              ...
              <div class="flex min-w-0 flex-col">
                <span class="capitalize"
                  >{entry.phase} — {entry.status.status}</span
                >
    ...
    <Card class="rounded-md border">
      <CardHeader
        ><CardTitle class="text-sm">Boot timeline</CardTitle></CardHeader
      >
      <CardContent>
