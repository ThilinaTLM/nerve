<script lang="ts">
  ...
  import type { ManagedSandboxRecord, StructuredLogLevel } from "@nervekit/contracts";
  ...
  $effect(() => {
    if (!["creating", "connecting", "starting", "reconnecting"].includes(lifecycle.state))
      return;
  ...
  function tone(level: StructuredLogLevel | undefined): "neutral" | "accent" | "warn" | "danger" {
      ...
      <div class="min-h-0 flex-1 overflow-auto">
        <ol class="divide-y">
          {#each lines as line (line.id)}
            <li class="grid grid-cols-[5.5rem_4.5rem_minmax(0,1fr)] gap-2 px-3 py-2 text-xs">
              ...
              <div class="min-w-0">
                <div class="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                  <span class={line.level === "error" ? "font-medium text-destructive" : "text-foreground"}>
                    {line.message}
                  </span>
