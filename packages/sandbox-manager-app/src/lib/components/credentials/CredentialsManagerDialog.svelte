...
const providerItems = [
  ...
  },
  {
    ...
    detail: "OAuth bundle import; manager refreshes",
  },
  ...
  { value: "github_pat", label: "GitHub token", detail: "Manual rotation" },
  {
    value: "github_app",
    label: "GitHub App",
    detail: "Installation token renewal",
  },
...
const selectedKind = $derived(
  providerItems.find((item) => item.value === providerKind),
);
...
async function saveProfile() {
  ...
  try {
    ...
    await store.createCredentialProfile({
      ...
      apiKey:
        !isOAuth && !providerKind.includes("github_app")
...
>
  <div class="flex max-h-svh flex-col gap-4 overflow-y-auto p-5">
        ...
        <CardContent class="flex items-center gap-3 p-4">
          ...
          <div>
            ...
            <p class="text-xs text-muted-foreground">
              No raw secrets in UI or sandbox config.
            </p>
          </div>
        </CardContent>
      </Card>
      <Card class="rounded-md border">
        <CardContent class="flex items-center gap-3 p-4">
          ...
          <div>
            ...
            <p class="text-xs text-muted-foreground">
              OAuth profiles renew before expiry.
            </p>
          </div>
        </CardContent>
      </Card>
      <Card class="rounded-md border">
        <CardContent class="flex items-center gap-3 p-4">
          ...
          <div>
            ...
            <p class="text-xs text-muted-foreground">
              Sandboxes cache only until manager deadlines.
            </p>
          </div>
        </CardContent>
      </Card>
    ...
    <Card class="rounded-md border">
      <CardHeader
        ><CardTitle class="text-sm">Add provider authentication</CardTitle
        ></CardHeader
      >
        ...
        <div class="grid gap-3 sm:grid-cols-2">
          <div class="flex flex-col gap-1">
            ...
            <SelectField
              items={providerItems}
              value={providerKind}
              onValueChange={(value) => (providerKind = value)}
            />
          </div>
          <div class="flex flex-col gap-1">
            ...
            <Input
              ...
              placeholder={selectedKind?.label ?? "Production provider"}
            />
          </div>
          {#if needsSite}
            <div class="flex flex-col gap-1">
    ...
    <Card class="rounded-md border">
      <CardHeader
        ><CardTitle class="text-sm">Managed profiles</CardTitle></CardHeader
      >
      <CardContent class="grid gap-2 text-sm md:grid-cols-2">
        ...
        {:else}
          {#each store.credentialProfiles as profile (profile.profileId)}
            <div
              class="flex items-start justify-between gap-3 rounded-md border bg-card px-3 py-2.5"
            >
              <div class="min-w-0">
              ...
              <Badge
                tone={profile.status === "configured" ? "good" : "warn"}
                size="xs"
              >
                {profile.status}
              </Badge>
