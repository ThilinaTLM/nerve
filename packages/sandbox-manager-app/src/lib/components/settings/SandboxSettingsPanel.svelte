...
// eslint-disable-next-line svelte/prefer-writable-derived -- Writable navigation state is synchronized from the shared center and also updated locally.
...
let providerKind =
  $state<SandboxManagerCredentialProviderKind>("anthropic_oauth");
...
$effect(() => {
  ...
  if (
    !activeSection.options.some(
      (option) => option.providerKind === providerKind,
    )
  ) {
...
function profilesForSection(
  ...
  return store.credentialProfiles.filter((profile) =>
    kinds.has(profile.providerKind),
  );
}
...
function supportsBrowserOAuth(option: ProviderOption | undefined): boolean {
  return Boolean(
    ...
    ["anthropic_oauth", "openai_codex_oauth", "github_copilot_oauth"].includes(
      option.providerKind,
    ),
  );
}
...
>
  ...
  <!-- eslint-disable-next-line @typescript-eslint/no-unused-vars -- SettingsShell requires the group snippet parameter. -->
  {#snippet panelActions(_group)}
    ...
    <Button
      variant="outline"
      size="sm"
      onclick={() => void store.refreshCredentials()}
    >
  ...
  {#snippet children(_group)}
    ...
    {:else}
      {#if saved || (error && !addDialogOpen)}
        <div
          class="settings-section-card settings-section-card-muted"
          role="status"
          aria-live="polite"
        >
          <div class="settings-section-body">
            {#if saved}
              <p
                class="flex items-center gap-2 rounded-md bg-success/10 p-2 text-xs text-success"
              >
              ...
              {#each sectionProfiles as profile (profile.profileId)}
                ...
                <article class="rounded-md border bg-card p-4">
                  ...
                  {#if fields.length > 0}
                    <div class="mt-3 grid gap-2 sm:grid-cols-2">
                      {#each fields as field (field.label)}
                        <div
                          class="min-w-0 rounded-md border bg-muted/30 px-2 py-1.5"
                        >
                  ...
                  {#if profile.lastError}
                    <p
                      class="mt-3 flex items-start gap-2 rounded-md bg-destructive/10 p-2 text-xs text-destructive"
                    >
                      <TriangleAlert class="mt-0.5 size-4 flex-none" />
...
>
    ...
    {#if selectedOption}
      <div class="grid gap-3 sm:grid-cols-2">
              ...
              >
                ...
                {#if oauthFlow.flow.status === "auth_url" && oauthFlow.flow.authUrl}
                  <Button
                    ...
                    size="sm"
                    onclick={() =>
                      oauthFlow.flow?.authUrl &&
                      oauthFlow.openExternal(oauthFlow.flow.authUrl)}
                  >
                    <ExternalLink class="size-4" /> Open login page
                  </Button>
                  {#if oauthFlow.flow.instructions}
                    <p class="text-xs text-muted-foreground">
                      {oauthFlow.flow.instructions}
                    </p>
                  {/if}
                {:else if oauthFlow.flow.status === "device_code" && oauthFlow.flow.deviceCode}
                  <div class="space-y-2">
                    <Button
                      ...
                      size="sm"
                      onclick={() =>
                        oauthFlow.flow?.deviceCode &&
                        oauthFlow.openExternal(
                          oauthFlow.flow.deviceCode.verificationUri,
                        )}
                    >
                    ...
                    <code
                      class="inline-flex rounded-md border bg-background px-2 py-1 font-mono text-sm font-semibold"
                      >{oauthFlow.flow.deviceCode.userCode}</code
                    >
                  </div>
                  ...
                  <div class="flex flex-wrap gap-2">
                    {#each oauthFlow.flow.options as option (option.id)}
                      <Button
                        ...
                        size="sm"
                        disabled={oauthFlow.busy}
                        onclick={() => void oauthFlow.selectOption(option.id)}
                      >
                        {option.label}
                      </Button>
                    {/each}
                ...
                {:else if oauthFlow.flow.status === "prompt"}
                  ...
                  >
                    ...
                    <Input
                      type="text"
                      autocomplete="off"
                      placeholder={oauthFlow.flow.placeholder ??
                ...
                {:else if oauthFlow.flow.status === "failed"}
                  <div class="space-y-2">
                    ...
                    <Button
                      ...
                      size="sm"
                      disabled={oauthFlow.busy}
                      onclick={() => void oauthFlow.restart()}
                    >
                      Try again
                    </Button>
                  </div>
                {:else if oauthFlow.flow.status === "succeeded"}
                  <p class="text-sm font-medium text-success">
                    Connected to {oauthFlow.flow.providerName}.
                  </p>
                {:else}
                  <p
        ...
        {:else if selectedOption.secretMode !== "none"}
          <div class="flex flex-col gap-1 sm:col-span-2">
            <div class="flex flex-wrap items-center justify-between gap-2">
              <Label
                >{selectedOption.secretMode === "apiKey"
                  ? "API key"
                  : secretLabel(selectedOption)}</Label
              >
              {#if supportsBrowserOAuth(selectedOption)}
                <Button
                  variant="ghost"
                  size="xs"
                  onclick={() => (manualOAuthImport = false)}
                >
                  Use browser login
                </Button>
              {/if}
            ...
            {#if selectedOption.multiline}
              <Textarea
                ...
                placeholder={secretPlaceholder(selectedOption)}
              />
            {:else}
              <Input
                bind:value={secretValue}
                type="password"
                placeholder={selectedOption.secretMode === "apiKey"
                  ? "Paste your API key"
                  : secretPlaceholder(selectedOption)}
              />
            {/if}
          </div>
        {/if}
        ...
        {#if hasAdvancedOptions(selectedOption)}
          <div
            class="space-y-3 rounded-md border bg-muted/20 p-3 sm:col-span-2"
          >
            <Button
              variant="ghost"
              size="sm"
              ...
              <div class="grid gap-3 sm:grid-cols-2">
                ...
                {#if selectedOption.kind === "model_provider"}
                  <div class="flex flex-col gap-1">
                    ...
                    {:else}
                      <Input
                        ...
                        placeholder={selectedOption.defaultModel ?? "model-id"}
                      />
                    {/if}
                  </div>
                  <div class="flex flex-col gap-1">
                  ...
                  <div class="flex flex-col gap-1">
                    ...
                    <Input
                      bind:value={api}
                      placeholder="builtin provider default"
                    />
                  </div>
                  <div class="flex flex-col gap-1">
                  ...
                  <div class="flex flex-col gap-1">
                    ...
                    <Textarea
                      ...
                      placeholder={JSON.stringify({ "X-Title": "Nerve" })}
                    />
                  </div>
                  <div class="flex flex-col gap-1">
                    <Label>Compat JSON</Label>
                    <Textarea
                      bind:value={compatJson}
                      class="min-h-20 font-mono text-xs"
                      placeholder={JSON.stringify({ supportsStore: false })}
                    />
                  </div>
                  <div class="flex flex-col gap-1 sm:col-span-2">
                    <Label>Provider options JSON</Label>
                    <Textarea
                      bind:value={providerOptionsJson}
                      class="min-h-20 font-mono text-xs"
                      placeholder={JSON.stringify({ routing: { order: [] } })}
                    />
                  </div>
                {/if}
                ...
                {#if supportsBrowserOAuth(selectedOption) && !manualOAuthImport}
                  <div class="sm:col-span-2">
                    <Button
                      variant="outline"
                      size="sm"
                      onclick={() => (manualOAuthImport = true)}
                    >
  ...
  {#snippet footer()}
    {#if selectedOption}
      <Button variant="ghost" size="sm" onclick={() => void closeAddDialog()}
        >Cancel</Button
      >
      {#if useBrowserOAuth(selectedOption)}
        {#if oauthFlow.flow?.status === "prompt"}
          <Button
            size="sm"
            disabled={oauthFlow.busy ||
              (!oauthFlow.flow.allowEmpty &&
                oauthFlow.promptValue.trim().length === 0)}
      ...
      {:else}
        <Button
          variant="outline"
          size="sm"
          onclick={() => applyOptionDefaults()}>Clear</Button
        >
        <Button
          size="sm"
