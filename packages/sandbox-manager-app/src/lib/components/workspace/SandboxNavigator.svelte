...
>
  {#snippet groupActions(group)}
    {@const record = store.sandboxes.find(
      (item) => item.sandboxId === group.id,
    )}
  ...
  {#snippet itemTooltip(item)}
    ...
    <span class="tt-row"
      ><span class="tt-key">status</span>{item.statusLabel}</span
    >
    {#if item.metadata?.mode}
      <span class="tt-row"
        ><span class="tt-key">mode</span>{item.metadata.mode}</span
      >
    {/if}
    {#if item.metadata?.updated}
      <span class="tt-row"
        ><span class="tt-key">updated</span>{item.metadata.updated}</span
      >
    {/if}
  {/snippet}
</ConversationGroupNavigator>
