...
const pendingWaits = $derived(
  waits.filter((wait) => wait.status === "waiting"),
);
...
function extractLatestContextFiles(
    ...
    <div class="flex flex-col gap-3">
      ...
      <dl class="grid grid-cols-[auto_minmax(0,1fr)] gap-x-3 gap-y-1.5">
        ...
        <dd class="truncate text-xs">
          {record.observedState} → {record.desiredState}
        </dd>
      ...
      <div class="flex flex-wrap items-center gap-1.5">
        <Button
          size="xs"
          variant="outline"
          onclick={() => onOpenDiagnosticTab?.("logs")}
          ><Terminal class="size-3.5" /> Logs</Button
        >
        <Button
          size="xs"
          variant="outline"
          onclick={() => onOpenDiagnosticTab?.("events")}
          ><FileClock class="size-3.5" /> Events</Button
        >
  ...
  <PanelSection title="Context" icon={Layers} bind:open={contextOpen}>
    <div class="flex flex-col gap-3">
      <dl class="grid grid-cols-[auto_minmax(0,1fr)] gap-x-3 gap-y-1.5">
        ...
        <dd class="truncate text-xs">
          {controls
            ? `${controls.mode} · ${controls.permissionLevel} · ${controls.thinkingLevel}`
            : "—"}
        </dd>
        <dt class="text-xs text-muted-foreground">Conversation</dt>
        <dd class="truncate font-mono text-xs">
          {detail?.selectedConversationId ?? richState?.conversationId ?? "—"}
        </dd>
        ...
        <dd class="truncate text-xs">
          {skills.length > 0
            ? `${visibleSkills.length}/${skills.length} model-visible`
            : "not reported"}
        </dd>
      </dl>
        ...
        <div class="flex flex-col gap-1.5">
          <div class="flex items-center justify-between gap-2 text-xs">
            ...
            <span class="font-mono tabular-nums"
              >{Math.round(contextPercent)}%{contextTokens
      ...
      {#if contextFiles.length > 0}
        <div
          class="-mx-3 -mb-2.5 flex max-h-40 flex-col overflow-y-auto border-t pt-1"
        >
          {#each contextFiles.slice(0, 6) as file (file.path)}
            <div class="flex items-center gap-2 px-3 py-1.5 text-xs">
              <Badge tone={file.included ? "good" : "neutral"} size="xs"
                >{file.included ? "in" : "out"}</Badge
              >
              <span class="min-w-0 flex-1 truncate font-mono" title={file.path}
                >{file.path}</span
              >
              {#if file.bytes !== undefined}<span class="text-muted-foreground"
                  >{displayBytes(file.bytes)}</span
                >{/if}
            </div>
          {/each}
        </div>
      {:else}
        ...
        >
          Context files are not reported yet.
        </p>
      {/if}
    </div>
  </PanelSection>
  ...
  <PanelSection title="Config & setup" icon={FileCode2} bind:open={configOpen}>
    <div class="flex flex-col gap-2">
      <dl class="grid grid-cols-[auto_minmax(0,1fr)] gap-x-3 gap-y-1.5">
        ...
        <dd class="truncate font-mono text-xs">
          {detail?.latestSession?.sessionId ?? "—"}
        </dd>
      </dl>
      <Button
        size="xs"
        variant="outline"
        onclick={() => onOpenDiagnosticTab?.("config")}>Open config YAML</Button
      >
    </div>
  </PanelSection>
</div>
