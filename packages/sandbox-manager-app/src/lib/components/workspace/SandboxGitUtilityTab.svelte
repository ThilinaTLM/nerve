...
function setupTone(
  status_: StartupSetupStatus | undefined,
...
function formatSetup(status_: StartupSetupStatus | undefined): string {
  ...
  return status_.configured
    ? status_.status
    : `not configured · ${status_.status}`;
}
...
<div class="flex flex-col gap-2 p-2">
  ...
  {#if connected && (repos.length > 0 || loadingRepos || overview)}
    <GitRepoBranchSection
      ...
      onSwitchBaseAndPull={(repo) =>
        void remoteAction(repo, "switchBaseAndPull")}
    />
    ...
    <GitPrSection
      ...
      selectedRepoHasGithubRemote={Boolean(
        selectedRepoSummary?.hasGithubRemote,
      )}
      {loadingPrs}
    ...
    <div class="flex flex-col gap-3">
      <div class="flex flex-wrap gap-1.5">
        <Badge tone={setupTone(status?.setup?.git)} size="xs"
          >git: {formatSetup(status?.setup?.git)}</Badge
        >
        <Badge tone={setupTone(status?.setup?.github)} size="xs"
          >github: {formatSetup(status?.setup?.github)}</Badge
        >
      </div>
