...
{#if showHtml}
  <div class={`code-view ${className}`} class:wrap-lines={wrap} style={codeViewStyle}>{@html html}</div>
{:else}
  <pre class={`code-view plain ${className}`} class:wrap-lines={wrap} style={codeViewStyle}><code>{#each textLines as line, index (index)}{@const lineNumber = lineStart + index}<span class={lineNumber === targetLine ? "code-line file-target-line" : "code-line"} data-file-line={lineNumber}>{line}</span>{/each}</code></pre>
{/if}
