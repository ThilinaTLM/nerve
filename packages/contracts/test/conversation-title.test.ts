...
describe("deriveConversationTitle", () => {
  ...
  it("removes file paths and urls", () => {
    assert.equal(
      deriveConversationTitle(
        "Please fix packages/workbench-app/src/App.svelte:123 and https://example.test/trace. The tabs are broken.",
      ),
      "The tabs are broken.",
    );
  });
  ...
  it("falls back to file review for file-only prompts", () => {
    assert.equal(
      deriveConversationTitle(
        "Please review packages/workbench-app/src/App.svelte",
      ),
      "File Review",
    );
  });
  ...
  it("falls back to file review for code/path-only prompts", () => {
    assert.equal(
      deriveConversationTitle(
        "/home/user/Projects/nerve/packages/workbench-app/src/App.svelte\n```ts\nconst x = 1;\n```",
      ),
      "File Review",
    );
  });
