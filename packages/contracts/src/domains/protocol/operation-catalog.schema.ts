/* eslint-disable max-lines -- Protocol operation catalog intentionally centralizes v1 method names and schemas. */
import { z } from "zod";
...
export type OperationIdempotency = z.infer<typeof operationIdempotencySchema>;
...
} as const satisfies Record<OperationName, OperationDefinition>;
...
export function allOperationDefinitions(): OperationDefinition[] {
...
function def(
  ...
  return {
    ...
    kind,
    idempotency,
    allowedTargetRoles: method.startsWith("sandbox.")
      ? ["sandbox_manager"]
      : ["workbench_server", "sandbox_agent"],
    requiredCapability: `operation.${method}`,
  };
}
