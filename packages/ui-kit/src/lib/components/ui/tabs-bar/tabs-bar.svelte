...
<Tabs.Root
  bind:value
  class={cn("tabs-bar-root min-w-0", className)}
  {onValueChange}
>
  <Tabs.List
    variant="line"
    aria-label={ariaLabel}
    class="tabs-bar-list w-full justify-start overflow-x-auto overflow-y-hidden"
  >
    {#each tabs as tab (tab.value)}
      ...
      >
        ...
        {/if}
        {#if tab.count}
          <span class={cn("tabs-bar-count", tab.icon && "tabs-bar-icon-count")}
            >{tab.count}</span
          >
        {/if}
      </Tabs.Trigger>
