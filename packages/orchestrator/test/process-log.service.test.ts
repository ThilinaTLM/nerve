...
import {
  ...
  ProcessLogService,
} from "../src/domains/processes/process-log.service.js";
...
describe("process log service line buffering", () => {
  ...
  it("increments log sequence only for emitted lines", async () => {
    ...
    const cursor = createProcessLogCursor(5);
...
async function createFixture(): Promise<{
  ...
  cursor: ReturnType<typeof createProcessLogCursor>;
  emitted: ProcessLogEvent[];
  onLog: (event: ProcessLogEvent) => Promise<void>;
}> {
  const root = await mkdtemp(join(tmpdir(), "nerve-process-log-"));
  roots.push(root);
  const id = createId("proc");
  const dir = join(root, "proc", id);
  ...
  const emitted: ProcessLogEvent[] = [];
  const service = new ProcessLogService(new EventBus(root));
  return {
    record,
    service,
    cursor: createProcessLogCursor(),
    emitted,
    onLog: async (event) => {
      emitted.push(event);
    },
