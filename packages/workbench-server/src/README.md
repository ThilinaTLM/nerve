...
- `adapters/protocol/` owns WebSocket dispatch, snapshots, idempotency, the verified operation registry, and thin domain-grouped handlers. Each handler group declares a narrow compile-time context and uses composed `RuntimeServices`; process hydration and shutdown remain private to `RuntimeLifecycle`.
