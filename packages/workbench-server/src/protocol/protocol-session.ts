  ...
  private handleHello(message: NerveMessage): void {
    ...
    this.send(
      "welcome",
      {
        sessionId: this.sessionId,
        acceptingPeer: {
          ...orchestratorSource(this.state.daemonId),
          instanceId: this.state.daemonId,
          name: `Nerve Workbench Server ${version}`,
        },
        acceptedVersion: 1,
  ...
  private startHeartbeat(): void {
    this.#heartbeatTimer = setInterval(() => {
      this.send("heartbeat", {
        ...
        load: { eventQueueDepth: this.#pendingLive.length },
      });
