...
type GitMethodContext = Pick<WorkbenchOperationContext, "services">;
...
export const gitMethodHandlers: WorkbenchMethodHandlerMapFor<GitMethodContext> =
  defineGitMethodHandlers({
    ...
    "git.file.unstage": (state, params) =>
      state.services.git.unstageFile(
        params.projectId,
        repo(params),
        params.path,
      ),
    "git.file.discard": (state, params) =>
      state.services.git.discardFile(
        params.projectId,
        repo(params),
        params.path,
      ),
    ...
    "github.pr.commits.get": (state, params) =>
      state.services.git.prCommits(
        params.projectId,
        repo(params),
        params.number,
      ),
    "github.pr.checks.get": (state, params) =>
      state.services.git.prChecks(
        params.projectId,
        repo(params),
        params.number,
      ),
