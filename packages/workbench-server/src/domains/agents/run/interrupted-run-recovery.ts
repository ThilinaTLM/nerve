...
const startedType = "run.started";
const terminalTypes = new Set(["run.completed", "run.failed", "run.suspended"]);
 ...
 * previous daemon process died, and publish a durable `run.failed`
...
export async function recoverInterruptedRuns(
  ...
  for (const run of active.values()) {
    ...
    await deps.events.publish("run.failed", {
      agentId: run.agentId,
      projectId: run.projectId,
      runId: run.runId,
      conversationId: run.conversationId,
