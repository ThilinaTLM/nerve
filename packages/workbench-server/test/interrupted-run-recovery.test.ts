...
describe("recoverInterruptedRuns", () => {
  it("fails runs that started but never reached a terminal event", async () => {
    ...
    bus.subscribe((evt) => {
      if (evt.type === "run.failed") failed.push(evt);
    ...
    const events: EventEnvelope[] = [
      ...
      event(2, "run.completed", { runId: "run_done" }),
      event(3, "run.started", {
        runId: "run_stuck",
        agentId: "agent_b",
        projectId: "proj_b",
      ...
      event(5, "run.suspended", { runId: "run_suspended" }),
    ];
  ...
  it("does nothing when all runs already reached a terminal state", async () => {
    ...
    const events: EventEnvelope[] = [
      event(1, "run.started", { runId: "run_1" }),
      event(2, "run.failed", { runId: "run_1" }),
    ];
