...
- Own concrete HTTP/WebSocket adapters, storage implementations, daemon composition, host use cases, process drivers, and workbench-specific domain adapters.
...
- Keep host harness construction in `src/domains/agents/run/harness-factory.ts`, host tool composition in `src/domains/tools/host-tool-factory.ts`, and OS process drivers under `src/domains/tasks/process-runtime/`.
