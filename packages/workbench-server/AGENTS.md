...
- Keep host harness construction in `src/domains/agents/run/harness-factory.ts` and host tool composition in `src/domains/tools/host-tool-factory.ts`. Native process lifecycle belongs in `@nervekit/native`; platform port discovery belongs behind `src/domains/tasks/task-port-inspector.ts`.
