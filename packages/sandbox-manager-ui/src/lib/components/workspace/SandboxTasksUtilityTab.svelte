<script lang="ts">
  ...
  import { writeClipboardText } from "@nervekit/shared-ui/core/clipboard";
  import { notify } from "@nervekit/shared-ui/core/notify";
  import TaskUtilityPanelView from "@nervekit/shared-ui/tasks/TaskUtilityPanelView.svelte";
  ...
  $effect(() => {
    ...
    if (connected) void store.refreshSandboxTasks(record.sandboxId).catch(() => undefined);
  });
...
<TaskUtilityPanelView
  unavailableMessage={connected ? undefined : "Pinned commands are available while stopped. Start or reconnect the sandbox to run commands and manage live tasks."}
  ...
  onUpdatePinned={(command, input) => updatePinned(command as SandboxPinnedCommand, input)}
