<script lang="ts">
  ...
  const hasRuntimeMetrics = $derived(
    Boolean(snapshot || detail?.status?.runs || conversations.length || agents.length),
  );
  ...
  const connectionRows = $derived([
    ...
    { label: "Last heartbeat", value: formatDate(detail?.status?.connectivity?.lastHeartbeatAt) },
...
<div class="h-full overflow-auto bg-background p-4">
  <div class="mx-auto flex max-w-5xl flex-col gap-3">
    <section class="rounded-md border bg-card p-4">
      <div class="flex flex-wrap items-start justify-between gap-4">
        <div class="min-w-0 flex-1">
          ...
          <p class="mt-2 text-sm font-medium text-foreground">{lifecycle.headline}</p>
          <p class="mt-0.5 max-w-2xl text-sm leading-relaxed text-muted-foreground">
          ...
          <div class="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
            ...
            {#if lifecycle.activeStage}
              <span aria-hidden="true">·</span>
              <span>{progress.completed}/{progress.total} startup steps complete</span>
            {/if}
          </div>
        </div>
        ...
        <div class="flex items-center gap-1.5">
          ...
          {:else if lifecycle.primaryAction === "start"}
            <Button size="sm" onclick={() => void store.startSandbox(record.sandboxId)}>
          ...
          {/if}
          <Button size="icon-sm" variant="ghost" ariaLabel="Refresh summary" onclick={refresh}>
      ...
      {#if lifecycle.issue && (lifecycle.state === "failed" || lifecycle.state === "degraded")}
        <div class="mt-3 flex items-start gap-2.5 rounded-md border bg-muted/30 p-3">
          <TriangleAlert class="mt-0.5 size-4 flex-none text-destructive" />
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2">
              <p class="text-sm font-medium">
                {lifecycle.issue.stage ? `${lifecycle.issue.stage} startup issue` : "Sandbox issue"}
              </p>
          ...
          <Button size="xs" variant="outline" onclick={openLogs}>Open logs</Button>
        </div>
      {/if}
    </section>
    ...
    <section class="flex flex-wrap items-center gap-1 rounded-md border bg-card px-2 py-1.5">
      ...
      <Button
        size="sm"
        variant={lifecycle.state === "failed" || lifecycle.state === "starting" ? "outline" : "ghost"}
        onclick={openLogs}
      >
      ...
      <Button
        size="sm"
        variant="ghost"
        onclick={() => store.openWorkspaceDiagnosticTab(record.sandboxId, "config")}
      >
      ...
      <Button
        size="sm"
        variant="ghost"
        onclick={() => store.openWorkspaceDiagnosticTab(record.sandboxId, "events")}
      >
