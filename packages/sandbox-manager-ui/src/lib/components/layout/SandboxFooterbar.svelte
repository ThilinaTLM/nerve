<script lang="ts">
  ...
  const pendingOps = $derived(
    Object.values(store.pendingOperations).filter(
      (operation) => operation.status === "pending" && (!operation.sandboxId || operation.sandboxId === sandboxId),
    ).length,
  );
...
>
  ...
  {#snippet right()}
    {#if !phone}
      {#if pendingOps > 0}
        <span class="footer-chip" title="Pending operations"><LoaderCircle size={12} strokeWidth={2.1} /> {pendingOps}</span>
      {/if}
      {#if pendingReviewCount > 0}
        <span class="footer-chip warn" title="Pending review"><TriangleAlert size={12} strokeWidth={2.1} /> {pendingReviewCount}</span>
      {/if}
    {/if}
    <span class="footer-chip" title={`Connection: ${store.connection}`}>
      <StatusDot tone={connectionTone} pulse={store.connection === "connecting" || store.connection === "reconnecting"} size="xs" />
