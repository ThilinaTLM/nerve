...
{:else}
  <div class="terminal-output rounded-sm border bg-sidebar px-2.5 py-1.5 font-mono text-xs leading-[1.22] text-sidebar-foreground">
    {#each visible as event (event.seq)}
      <div class="grid grid-cols-[3.5rem_minmax(0,1fr)] gap-2" class:text-warning={event.level === "warn"} class:text-destructive={event.level === "error"}>
        ...
        <span class="whitespace-pre-wrap break-words"><TerminalText text={event.line || "\u00A0"} stream={event.stream} level={event.level} /></span>
      </div>
    {/each}
  </div>
{/if}
