...
describe("withInitialZoomLevel", () => {
  ...
  it("leaves the URL unchanged when settings were unavailable", () => {
    const url = "http://127.0.0.1:3747";
    assert.equal(withInitialZoomLevel(url, undefined), url);
    assert.equal(withInitialZoomLevel(url, 1.5), url);
  });
});
