...
<DocsScreenshot
  light={historyLight}
  dark={historyDark}
  alt="The Nerve conversation history graph zoomed into a cluster where several branches diverge, with the selected entry shown in the details pane"
  caption="What to notice: each fork is a point where work continued from an earlier entry. The original lineage is still there — nothing was overwritten to make room for the new branch."
