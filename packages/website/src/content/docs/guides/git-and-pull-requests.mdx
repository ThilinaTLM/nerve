...
<DocsScreenshot
  light={gitLight}
  dark={gitDark}
  alt="Nerve Git panel showing repository status, staged and unstaged changes, and commit history"
  caption="Repository state and scoped actions stay visible beside the conversation, but destructive discard still requires confirmation."
...
<DocsScreenshot
  ...
  alt="Nerve pull request tab showing a GitHub pull request conversation, checks, commits, and changed files"
  caption="The pull request tab brings review context, checks, commits, and changed files together before checkout or merge actions."
  widths={[480, 768, 1024, 1440]}
  sizes="(min-width: 72rem) 48rem, calc(100vw - 3rem)"
/>
