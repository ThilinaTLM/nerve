...
<DocsScreenshot
  light={gitLight}
  dark={gitDark}
  alt="Nerve Git panel showing four repositories discovered in one project, the selected repository's branch, and its staged and unstaged changes"
  caption="What to notice: every repository under the project root gets its own branch and working-tree state, side by side with the conversation. Destructive discard still asks first."
...
<DocsScreenshot
  ...
  alt="Nerve pull request view showing the pull request summary, mergeability, checks, commits, and changed files beside the list of open pull requests"
  caption="What to notice: review context, checks, commits, and changed files sit in the workbench, so checkout and merge decisions happen without a browser round trip."
  widths={[480, 768, 1024, 1440]}
  sizes="(min-width: 72rem) 48rem, calc(100vw - 3rem)"
/>
