...
Use finite Bash for commands that should finish. Use `task_start` for a known server or watcher. Nerve can automatically promote a Bash command that remains active. The agent can continue independent work or finish its turn instead of polling. Agent-started tasks subscribe the agent to terminal completion: when one terminates, Nerve adds a `task_event` and starts a fresh agent turn to inspect it. A task is separate from an agent todo: tasks supervise processes, while todos describe work inside a conversation.
...
A task can define readiness by URL, first detected URL, or output pattern. The agent receives asynchronous status rather than polling; status and log tools remain available for on-demand diagnosis. Dedicated views provide status and bounded streaming logs with cursor, error, warning, and first-failure modes. Runtime can be capped at up to 24 hours.
...
The Tasks panel and task-output tab let you switch between sibling runs, restart a saved task, cancel a run, load earlier log output, clean up terminal history, and prune old runs. The project switcher also includes running task counts in its activity state, so work in another project remains visible. The agent uses `task_control` with `action: "stop"` or `action: "restart"` for one task at a time. **Force kill** is available only when Nerve has verified the process identity (or a run is stuck stopping); it is intentionally unavailable for `recovery_unknown` records. Removing a run's history does not remove the durable task definition.

<DocsScreenshot
  light={tasksLight}
  dark={tasksDark}
  alt="The Nerve Tasks panel listing saved task definitions and their recent runs beside the conversation that started them"
  caption="What to notice: a task definition keeps its identity while each run stays separately inspectable, with its readiness, status, and retained output."
...
Task terminal changes become deduplicated `task_event` system entries in the conversation. Both auto-promoted Bash commands and explicit agent `task_start` processes subscribe the originating agent, so a terminal event starts a fresh continuation turn if the originating run already finished. Tasks started outside an agent conversation remain notification-only. This is separate from desktop/browser toast notifications.
