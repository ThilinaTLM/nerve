...
<DocsScreenshot
  light={tasksLight}
  dark={tasksDark}
  alt="Nerve Tasks panel and task output tab showing supervised process state and bounded logs"
  caption="A durable task definition keeps its identity while individual runs, readiness, status, and historical output remain inspectable."
