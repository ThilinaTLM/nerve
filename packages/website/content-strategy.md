...
> **Editorial status (2026-09-25):** Public copy was re-audited against v0.31.1 at HEAD. The current corpus reflects editable file tabs, seven dock views, provider-neutral image generation, persistent async developer teammates, grouped agent monitoring, conversation capability overrides, and the mobile shell. The docs use a guided `/start/` path plus a grouped `/guides/` workflow index; content-integrity checks remain in place. A new production screenshot capture remains a separate follow-up.
...
| Movable/resizable docks                       | Shape the workbench around the task                                                                    | `panel-registry.ts`; `WorkbenchShell.svelte`; `shell-layout.ts`                              | Seven dock views: Conversations, Files, Tasks, Notes, Git, Pull Requests, Context. Layout persists locally.                                                | P1       |
| Responsive/mobile workbench                   | Use the same UI in browser, tablet, or phone                                                           | `MobileWorkbenchHost.svelte`; mobile navigation and screen primitives                        | Compact below 1024px; below 640px, Inbox, Chats, Workspace, and More become the primary navigation. Mobile access still needs a reachable, secured daemon. | P1       |
...
| File view and editor                          | Inspect files and make focused text edits                                                              | `FilePane.svelte`; filesystem routes                                                         | Small regular in-project text files are revision-checked and editable; external, linked, binary, truncated, and >1 MiB files remain read-only.             | P1       |
...
| Resource and skill loading                    | Add project/user and opt-in Built-in Nerve guidance without modifying the harness | `@nervekit/skills`; `resource-loader.ts`; skill tests | Project definitions win by name; built-ins are disabled by default; `.nerve`/`.agents` precedence must be documented exactly. Legacy `.pi` is not loaded. Changes apply on subsequent runs.                                                                                                                  | P1       |
...
| Persistent developer teammates | Delegate autonomous implementation and retain context for follow-ups | async-subagent service/tools; Context agents UI    | Disabled by default; named teammates share the worktree, do not inherit the lead transcript, and allow at most four active assignments per lead.                                           | P1       |
...
| `/guides/files-context-notes/`   | File viewing/editing, save conflicts, line targeting/wrap/Markdown, context/agents, project notes                                      | Workbench → resources and delegation                  | Combined panel screenshot; editable-file boundaries and truncation warning                          |
...
| `/guides/skills-and-resources/`  | AGENTS/SYSTEM/context/skills locations, Built-in Nerve skills, precedence, toggles, effective next-run behavior                        | Project → resource reference, Agent Browser           | Skills settings + precedence diagram; project files are trusted instructions                        |
