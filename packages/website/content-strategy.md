...
| Read-only, supervised, autonomous permissions | Match agent authority to risk tolerance                    | policy evaluator and tests                        | A supervision engine combines manifest/argument risk, hard constraints, permission level, and user preferences. Read-only blocks non-read risks; supervised asks unless read autoapproval or a risk-bound grant applies; autonomous allows after hard guards. | P0       |
