...
function resultDiff(
  result: Awaited<ReturnType<typeof executeSmartEdit>>,
): string {
...
describe("smart_edit executor", () => {
  it("normalizes convenience and JSON-string operations", () => {
    assert.deepEqual(
      normalizeSmartEditArgs({ oldText: "a", newText: "b" }).operations,
      [
        {
          type: "replace_text",
          oldText: "a",
    ...
    assert.deepEqual(
      normalizeSmartEditArgs({ patch: "@@ -1 +1 @@\n-a\n+b\n" }).operations,
  ...
  it("rejects invalid operation shapes", () => {
    assert.throws(
      () => normalizeSmartEditArgs({ operations: [] }),
      /at least one operation/,
    );
    assert.throws(
      () => normalizeSmartEditArgs({ operations: [null] }),
      /must be an object/,
    );
    assert.throws(
      () =>
  ...
  it("applies exact text operations and records operation metadata", async () => {
    ...
    const result = await executeSmartEdit(
      {
        path: "example.txt",
        operations: [
  ...
  it("supports dry-run previews without writing", async () => {
    ...
    assert.match(result.content ?? "", /Previewed smart edit/);
