...
import type { ExecutionEnv, FileInfo, FileKind } from "./contracts.js";
...
export class NodeExecutionEnv implements ExecutionEnv {
  ...
  > {
    ...
    return await new Promise((resolvePromise) => {
      ...
      child.on("close", (code) => {
        ...
        if (timedOut) {
          settle(
            err(new ExecutionError("timeout", `timeout:${options?.timeout}`)),
          );
          return;
        }
        if (options?.abortSignal?.aborted) {
  ...
  async readTextFile(
    ...
    try {
      return ok(
        await readFile(resolved, { encoding: "utf8", signal: abortSignal }),
      );
    } catch (error) {
  ...
  async readTextLines(
    ...
    try {
      ...
      const afterReadAbort = abortResult<string[]>(
        options?.abortSignal,
        resolved,
      );
